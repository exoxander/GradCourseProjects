...
//modified to return the edge types for each vertex as well
std::string DepthFirstSearch::str()  const{
  std::ostringstream sout;
  for (int u = 0; u < g.V(); u++) {
    sout << u << ": (" << v_attributes[u].time[0]
         << '/' << v_attributes[u].time[1] << ")"
         << " - " << v_attributes[u].color
         << " - " << v_attributes[u].parent
         << " - " << v_attributes[u].component << '\n';


    //u = self, v = other
    //loop through all edges in the adj list of u and check edge type
    std::list<int> u_adj = g.adj(u);
    for (std::list<int>::iterator v = u_adj.begin(); v != u_adj.end(); ++v) {

        sout << ' ' << u << " -> " << *v << ": ";

        //if tree edge (if u == v.parent)
        if (u == v_attributes[*v].parent) {
           sout << 'T';
        }

        //if forward edge (if u.d < v.d && u.f > v.f)
        else if (v_attributes[u].time[0] < v_attributes[*v].time[0] && v_attributes[u].time[1] > v_attributes[*v].time[1]) {
            sout << 'F';
        }

        //if back edge (if u.d > v.d && u.f < v.f)
        else if (v_attributes[u].time[0] > v_attributes[*v].time[0] && v_attributes[u].time[1] < v_attributes[*v].time[1]) {
            sout << 'B';
        }

        //if none of the above: cross edge
        else {
            sout << 'C';
        }
        sout << " | ";
